...
动手改 Editor / 组件 / 脚本桥之前，至少阅读仓库内（文档站点外）规范文件：

1. `.trellis/spec/product-ux.md` — 面向用户的 Inspector；必须用 `InspectorControls`
2. `.trellis/spec/agent-workflow.md` — 命名、成套交付、先问后改、Trellis 任务

索引见仓库根下 `.trellis/spec/index.md`。
...
- 仓库 Spec 索引：`.trellis/spec/index.md`（位于文档站点之外的仓库路径）
